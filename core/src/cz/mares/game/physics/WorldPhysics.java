...
public class WorldPhysics {
    ...
    public void setLevelData(LevelData levelData){
        ...
        TiledMapTileLayer wallLayer = (TiledMapTileLayer) levelData.getTerrain().getLayers().get(LevelLoader.WALL_LAYER);
    ...
    private boolean checkTerrainCollision(Rectangle position, Rectangle rectangle, Vector2 resultVelocity, Direction direction) {
        ...
        int fromXOffset = direction == Direction.RIGHT ? 0 : LevelLoader.TILE_SIZE * COLLISION_DEPTH_CHECK;
        int toXOffset = direction == Direction.LEFT ? 0 : LevelLoader.TILE_SIZE * COLLISION_DEPTH_CHECK;
        int fromYOffset = direction == Direction.UP ? 0 : LevelLoader.TILE_SIZE * COLLISION_DEPTH_CHECK;
        int toYOffset = direction == Direction.DOWN ? 0 : LevelLoader.TILE_SIZE * COLLISION_DEPTH_CHECK;

        int fromTilesX = Math.max((leftX - fromXOffset) / LevelLoader.TILE_SIZE, 0);
        int toTilesX = Math.min((rightX + toXOffset) / LevelLoader.TILE_SIZE, terrainPositionWidth - 1);
        int fromTilesY = Math.max((bottomY - fromYOffset) / LevelLoader.TILE_SIZE, 0);
        int toTilesY = Math.min((topY + toYOffset) / LevelLoader.TILE_SIZE, terrainPositionHeight - 1);
